...
% use measured protein content from Valgepea et al. (2013) and assign the median to
% the remaining conditions
% μ=0.11 h-1	μ=0.21 h-1	μ=0.31 h-1	μ=0.40 h-1	μ=0.49 h-1
% P=61.0%        P=58.2%	P=55.4%     P=52.9%     P=50.4%
P(:) = NaN;
P(cellfun(@(x)find(ismember(condNames,x)),{...
    'GLC_CHEM_mu_0_11_V', 'GLC_CHEM_mu_0_21_V', 'GLC_CHEM_mu_0_31_V',...
    'GLC_CHEM_mu_0_40_V', 'GLC_CHEM_mu_0_49_V'})) = ...
    [0.61 0.582 0.554 0.529 0.504];
P(isnan(P)) = median(P,'omitnan');
...
save(fullfile('Results','lambda_cv','lambda_fitting_e_coli_median_ptot'),'relErr','errVar','sumsDelta','objVal',...
