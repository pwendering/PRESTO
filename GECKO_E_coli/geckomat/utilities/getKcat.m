...
if sum(pIdx, 'all')==0
    pIdx    = contains(ecModel.metNames,['prot_' protein]);
    if sum(pIdx, 'all')>1
        error(append(protein, ' is found in more than one metabolite. Ambigous ID'))
    elseif sum(pIdx, 'all')==0
        error(append(protein, ' not found'))
    end
end
